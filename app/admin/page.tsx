  ...
  React.useEffect(() => {
    fetch("/api/media")
      .then(async res => {
        if (!res.ok) {
          const err = await res.json();
          setMediaError(err.error || "Failed to fetch media");
          return;
        }
        return res.json();
      })
      ...
      .catch(() => setMediaError("Failed to fetch media"));
  }, []);
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#181a20", color: "#f7fafc", fontFamily: "Inter, sans-serif", padding: '0 1rem' }}>
      <h1 style={{ color: '#ffe082', fontSize: '2.2rem', margin: '36px 0 18px 0', textAlign: 'center' }}>Admin Dashboard</h1>
            ...
            <div style={{ maxWidth: 600, margin: '0 auto', background: '#23242b', border: '1.5px solid #7fd1b9', borderRadius: 16, boxShadow: '0 2px 16px #0004', padding: 24, marginBottom: 48 }}>
              {linksError ? (
